  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400">Checklist</label>
        <button
          type="button"
        ...
        {items.map((item, idx) => (
          <div key={item.id} className="flex items-center gap-1.5">
            <div className="flex flex-col -space-y-1">
              <button type="button" onClick={() => moveItem(idx, -1)} disabled={idx === 0} className="text-gray-300 dark:text-gray-600 disabled:opacity-30 hover:text-gray-600 dark:hover:text-gray-300">
                <ChevronUp className="w-3 h-3" />
              </button>
              <button type="button" onClick={() => moveItem(idx, 1)} disabled={idx === items.length - 1} className="text-gray-300 dark:text-gray-600 disabled:opacity-30 hover:text-gray-600 dark:hover:text-gray-300">
