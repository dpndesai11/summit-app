...
export default function CollapsibleCard({
  title, icon: Icon, iconColor = 'text-gray-500', badge, actions, defaultOpen = true, children,
}) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      ...
      >
        ...
        {badge != null && <span className="text-[11px] text-gray-400">{badge}</span>}
        {actions && (
          <div className="ml-auto flex items-center gap-2" onClick={e => e.stopPropagation()}>
            {actions}
          </div>
        )}
        <ChevronDown className={`w-4 h-4 text-gray-400 flex-shrink-0 transition-transform ${actions ? '' : 'ml-auto'} ${open ? 'rotate-180' : ''}`} />
      </button>
